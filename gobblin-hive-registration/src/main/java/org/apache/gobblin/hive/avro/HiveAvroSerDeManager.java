...
public class HiveAvroSerDeManager extends HiveSerDeManager {
  ...
  public HiveAvroSerDeManager(State props) throws IOException {
    ...
    if (props.contains(HiveRegistrationPolicyBase.HIVE_REGISTRATION_FS_URI)) {
      this.fs = FileSystem.get(URI.create(props.getProp(HiveRegistrationPolicyBase.HIVE_REGISTRATION_FS_URI)), HadoopUtils.getConfFromState(props));
    } else {
