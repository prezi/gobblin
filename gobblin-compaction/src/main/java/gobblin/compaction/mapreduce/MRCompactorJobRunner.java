...
import static gobblin.compaction.mapreduce.MRCompactor.COMPACTION_PREFIX;
import static gobblin.util.retry.RetryerFactory.RETRY_INTERVAL_MS;
import static gobblin.util.retry.RetryerFactory.RETRY_MULTIPLIER;
import static gobblin.util.retry.RetryerFactory.RETRY_TIME_OUT_MS;
import static gobblin.util.retry.RetryerFactory.RETRY_TYPE;
...
public abstract class MRCompactorJobRunner implements Runnable, Comparable<MRCompactorJobRunner> {
  ...
  private void moveTmpPathToOutputPath() throws IOException {
    ...
    final Path outputTmpPath = this.dataset.outputTmpPath();
